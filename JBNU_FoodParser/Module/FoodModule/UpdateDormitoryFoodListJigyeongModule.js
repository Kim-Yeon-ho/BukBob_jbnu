...
import  GetFood  from './GetDormitoryFoodListJigyeong.js';
...
const DbFoodListUpdateStart = ((foodArr) => {
  ...
  try {
    SetDbjigyeong(foodArr, 0, 4, 'Jigyeong'+dayState[0], '직영관', '조식');
    SetDbjigyeong(foodArr, 5, 9, 'Jigyeong'+dayState[1], '직영관', '중식');
    SetDbjigyeong(foodArr, 10, 14, 'Jigyeong'+dayState[2], '직영관', '석식');
...
const SetDbjigyeong = ((foodArr, start, end, docName, title, state)=>{
...
const UpdateFoodList = (()=>{
  GetFood().then((foodArr)=>{
    if(Array.isArray(foodArr) === false){
      ...
      EditLog('Update Complete Jigyeong');
    }
  });
